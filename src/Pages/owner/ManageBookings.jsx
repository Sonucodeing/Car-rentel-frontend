...
const ManageBookings = () => {
  ...
  const changeBookingStatus = async (bookingId, status)=>{
    try {
      const {data} = await axios.post(`${BASE_URL}/api/booking/change-status`,{bookingId, status})
      if(data.success){
...
fetchOwnerBooking()
  ...
  return (
    ...
    <Title title='Manage Bookings' subtitle='Track all customer booking, approve or cancel request, and manage booking statuses.'/>
        ...
        <table className='w-full border-collapse text-left text-sm text-gray-600'>
          ...
          <tbody>
            {booking.map((booking,index)=>(
              ...
              </select>):(<span className={`px-3 py-1 rounded-full text-xs font-semibold ${booking.status === 'confirmed' ? 'bg-green-100 text-green-500' : 'bg-red-100 text-red-500'}`}>{booking.status}</span>)}</td>
              </tr>
