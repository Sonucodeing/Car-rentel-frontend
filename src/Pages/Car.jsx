...
const [filteredCar, setFiltredCar] = useState([])
...
setFiltredCar(filtered)
}

const searchCarAvailablity = async () =>{
  const {data} = await axios.post(`${BASE_URL}/api/booking/check-availability`, {location: pickupLocation, pickupDate, returnDate})
  if(data.success){
    setFiltredCar(data.availableCars)
    if(data.availableCars.length === 0){
      toast('No cars Avaliable')
    }
    return  null
  }
}
...
useEffect(()=>{
 ...
 console.log('car from context: ', cars)
